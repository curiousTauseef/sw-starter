	...
	exports.default = {
	    init: function init() {
	        ...
	        promise.then(function (data) {
	            _this.insertHtml($instafeed, _this.createMobileHtml(data.posts));
	        }).then(function () {
	            _this.initMobileInstagramSlider();
	        }).then(function () {
	            $instafeed.addClass('loaded');
	        });
	    },
	    insertHtml: function insertHtml($el, html) {
	        $el.html(html);
	    },
	    createMobileHtml: function createMobileHtml(data) {
	        if (!data) return false;
	
	        var html = '<div class="instagram--mobile__slider">';
	        html += this.createMobileSliderHtml(data);
	        html += '</div>';
	
	        return html;
	    },
	    createMobileSliderHtml: function createMobileSliderHtml(data) {
	        var _this2 = this;
	
	        return data.map(function (item, key) {
	            return _this2.instagramItemHtml(key, item, false);
	        }).join('');
	    },
	    instagramItemHtml: function instagramItemHtml(key, data, insertTitleInGrid) {
