...
/*
* Reditect for the thank you page.
*/
function thankyou_redirect(){

    $house_arr = array(
        array(  'site_id'=> MIAMI_SITE_ID,
                'house_id'=> 410821,
                'site' => 'miami'),
        array(  'site_id'=> CHICAGO_SITE_ID,
                'house_id'=> 410822,
                'site' => 'chicago')
        );

    if(!empty($_GET['house'])){
        foreach ($house_arr as $key => $value) {
            if( $_GET['house'][0]==$value['house_id']){
            
                $args = $_GET;
                unset( $args['house'] );
                $get = http_build_query( $args );
                wp_redirect( '/'. $value['site'] .'/thank-you/?' . $get );
                exit();
...
add_action( 'template_redirect', 'thankyou_redirect' );

function thankyou_shortcode( $atts ) {
    ob_start();
    include(locate_template('templates/thank-you.php') );
    return ob_get_clean();
}
add_shortcode( 'thankyou', 'thankyou_shortcode' );


function assd_form_shortcode( $atts ) {
    $query_vars = [];
    if(!empty($_REQUEST)){
      $query_vars = $_REQUEST;
    }
    ...
    if(!empty( $atts ) ){
        $query_vars  = array_replace_recursive($query_vars ,$atts);
    }
    ...
    ob_start();
    include(locate_template('templates/assd_form.php') );
    return ob_get_clean();
}
add_shortcode( 'assd_form', 'assd_form_shortcode' );

// function assd_reserve_redirect(){
//     if(is_page('reserve') ){
//         if(empty($_REQUEST['house']) || empty($_REQUEST['cust'])){
//             wp_safe_redirect( home_url() );
//         }
//     }    
// }
// add_action( 'template_redirect', 'assd_reserve_redirect' );
